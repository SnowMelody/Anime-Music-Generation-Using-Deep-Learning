...
def get_midi_files(folder='transposed_midi_files/*.mid'):
	"""Get all midi files from specified folder.

	Parameters:
	------------
	folder: folder directory of midi files

	Returns:
	------------
	list_all_midi (list): list containing all midi files 
	"""
...
def get_notes(list_all_midi):
	"""For each midi file, get all notes and their attributes.

	Attributes are the note offset, pitch, and duration.
	...
	Returns:
	------------
	all_items (list of lists): list of lists, each sublist containing note 
	attributes for that midi file 
	"""
...
def process_part(part_to_process, note_info, offset):
	"""Further process the specified part for that midi file.

	Parameters:
	------------
	part_to_process (list): list for one of the 4 specified parts
	note_info (list): list containing the note midi value, duration, and pitch
	offset (float): current offset of the midi file

	Returns:
	------------
	part (list): list processed for the specified part
	"""
...
def get_four_parts(all_items):
	"""Process the melody, right middle, left middle, and bass parts.

	For each midi file, in each part, the note midi value, hold, and rest
	are placed in sublists. Their position/index are based on offset 
	increments of 0.25.
	...
	Returns:
	------------
	all_melody (list of lists): list of lists, each sublist containing 
	note midi value, hold, and rest for that midi file melody part
	all_rmid (list of lists): list of lists, each sublist containing 
	note midi value, hold, and rest for that midi file right middle part
	all_lmid (list of lists): list of lists, each sublist containing 
	note midi value, hold, and rest for that midi file left middle part
	all_bass (list of lists): list of lists, each sublist containing 
	note midi value, hold, and rest for that midi file bass part
	"""
...
def item_to_int(item):
	"""Create a dictionary that maps items to integers.

	Parameters:
	------------
	item (list): list containing the items to be mapped

	Returns:
	------------
	dictionary mapping items (key) to integers (value)
	"""
...
def int_to_item(item_dict):
	"""Create a dictionary that maps integers to items.

	Parameters:
	------------
	item_dict (dict): dictionary to be mapped

	Returns:
	------------
	dictionary mapping integers (key) to items (value)
	"""
...
def get_training_sequences(all_part, item_dict, seq_len):
	"""Get training sequences to feed into the deep neural network.

	Parameters:
	------------
	all_part (list of lists): lists of lists, each sublist containing 
	a midi file's specified part or relevant information
	item_dict (dict): dictionary mapping integers (key) to items (value)
	seq_len (int): length of input training sequence

	Returns:
	------------
	train_list(list of lists): lists of lists, each sublist containing
	input training sequences of fixed defined length
	target_list(list): list containing output target
	"""
...
def get_bar_count(all_part):
	"""Get counter conditioning information for each midi file.

	Midi files are in 4/4 time. The counter division is of a semiquaver
	note. Hence for 1 bar, the counter increments from 0 to 15, and
	repeats for subsequent bars.

	Parameters:
	------------
	all_part (list of lists): lists of lists, each sublist containing 
	a midi file's specified part 

	Returns:
	------------
	all_counter (list of lists): lists of lists, each sublist containing
	a midi file's counter information
	"""
...
def get_motifs(all_part, i_size=16, seq_len=64, max_count=3):
	"""Get motifs conditioning information for each midi file.

	Repeated motifs are multiple sections in music where notes are 
	exactly the same. A motif is first identified, then have its repetitions 
	searched for in the defined window size. For this function to be meaningful, 
	seq_len >= i_size*2, and max_count >=2. 

	Here, motifs are searched for in the melody part.
	i_size = 16 means the motif is 1 bar long (each semiquaver is 1/16 bar duration).
	seq_len = 64 means repetitions are searched within 4 bars of motif's occurrence.
	max_count = 3 means the motif's repetition is searched for another 2 times. 

	Parameters:
	------------
	all_part (list of lists): lists of lists, each sublist containing 
	a midi file's specified part
	i_size (int): window size for the motif
	seq_len (int): window size to search for repetitions for a motif
	max_count (int): maximum number of repetitions to be searched for a motif

	Returns:
	------------
	all_motifs (list of lists): lists of lists, each sublist containing
	a midi file's motifs information
	"""
...
def get_form(list_all_midi, form_list, all_part):
	"""Get form conditioning information for each midi file.

	The form_list is stored as a pickled file, obtained from
	FORM_INPUT.py.

	Parameters:
	------------
	list_all_midi (list): list containing all midi files 
	form_list (list of lists): lists of lists, each sublist containing
	a midi file's form information
	all_part (list of lists): lists of lists, each sublist containing 
	a midi file's specified part

	Returns:
	------------
	all_form (list of lists): lists of lists, each sublist containing
	a midi file's form information
	"""
...
def create_model(num_melody, num_rmid, num_lmid, num_bass, num_counter, num_motifs, num_form, seq_len, dropout):
	"""Create the deep neural network model.

	Parameters:
	------------
	num_melody (int): number of unique inputs for the melody part
	num_rmid (int): number of unique inputs for the right middle part
	num_lmid (int): number of unique inputs for the left middle part
	num_bass (int): number of unique inputs for the bass part
	num_counter (int): number of unique inputs for the counter 
	conditioning information
	num_motifs (int): number of unique inputs for the motifs 
	conditioning information
	num_form (int): number of unique inputs for the form 
	conditioning information
	seq_len (int): input training sequence length
	dropout (float): dropout value used in the dropout layer

	Returns:
	------------
	model: a Keras deep neural network model architecture
	"""
	...
	counter_input = tf.keras.layers.Input(shape = (seq_len,))
	motifs_input = tf.keras.layers.Input(shape = (seq_len,))
	form_input = tf.keras.layers.Input(shape = (seq_len,))
	...
	counter_embedding = tf.keras.layers.Embedding(input_dim = num_counter, output_dim = 8)(counter_input)
	motifs_embedding = tf.keras.layers.Embedding(input_dim = num_motifs, output_dim = 4)(motifs_input)
	form_embedding = tf.keras.layers.Embedding(input_dim = num_form, output_dim = 4)(form_input)
	merge_layer = tf.keras.layers.Concatenate()([melody_embedding, rmid_embedding, lmid_embedding, 
												bass_embedding, counter_embedding, motifs_embedding, form_embedding])
	...
	model = tf.keras.Model(inputs = [melody_input, rmid_input, lmid_input, bass_input, counter_input, motifs_input, form_input], 
...
def train_model(model, training_input, training_output, test_input, test_output):
	...
	------------
	model: a Keras deep neural network model architecture
	training_input (np array): the input training sequences for the training set
	training_output (np array): the output targets for the training set
	test_input (np array): the input sequences for the test set
	test_output (np array): the output targets for the test set
	...
	checkpoint_path = "training_checkpoints_model1/weights-{epoch:02d}-{loss:.4f}.ckpt"
	...
	with open('model1_history', 'wb') as f:
	    pickle.dump(saveObject, f)
...
model = create_model(num_melody, num_rmid, num_lmid, num_bass, num_counter, num_motifs, num_form, SEQ_LEN, dropout = 0.3)
model.compile(loss = 'sparse_categorical_crossentropy', metrics = ['sparse_categorical_accuracy'], optimizer = 'adam')
model.summary()
train_model(model, 
			[train_input_melody, train_input_rmid, train_input_lmid, train_input_bass, train_input_counter, train_input_motifs, train_input_form], 
			[train_output_melody, train_output_rmid, train_output_lmid, train_output_bass],
			[test_input_melody, test_input_rmid, test_input_lmid, test_input_bass, test_input_counter, test_input_motifs, test_input_form], 
			[test_output_melody, test_output_rmid, test_output_lmid, test_output_bass])

model.evaluate([test_input_melody, test_input_rmid, test_input_lmid, test_input_bass, test_input_counter, test_input_motifs, test_input_form],
				[test_output_melody, test_output_rmid, test_output_lmid, test_output_bass])
print('Done training model')


# PART 3: use trained model to generate notes and write midi files
def predict_notes(melody_seq, rmid_seq, lmid_seq, bass_seq, counter_seq, motifs_seq, form_seq):
	"""Use the deep neural network model to predict notes for each of the 4 parts.

	Parameters:
	------------
	melody_seq (np array): the input sequence for the melody part
	rmid_seq (np array): the input sequence for the right middle part
	lmid_seq (np array): the input sequence for the left middle part
	bass_seq (np array): the input sequence for the bass part
	counter_seq (np array): the input sequence for the counter information
	motifs_seq (np array): the input sequence for the motifs information
	form_seq (np array): the input sequence for the form information

	Returns:
	------------
	The argmax of the predicted note for each part (i.e note with the highest probability)
	"""

	predicted_melody, predicted_rmid, predicted_lmid, predicted_bass = model.predict([melody_seq, rmid_seq, lmid_seq, 
																					bass_seq, counter_seq, motifs_seq, form_seq])
...
def process_prediction_input(pred_input, predicted_note):
	"""Reshapes the input sequence for the next prediction to be made.

	Parameters:
	------------
	pred_input (np array): the input sequence
	predicted_note (int): the predicted note for current input sequence

	Returns:
	------------
	pred_input (np array): the reshaped input sequence
	"""
...
generated_rmid = []
generated_lmid = []
generated_counter = np.tile(train_input_counter[30], 3)
generated_motifs = np.tile(train_input_motifs[30], 3)
generated_form = np.tile(train_input_form[30], 3)
print(generated_counter)
print(generated_motifs)
print(generated_form)

for i in range(256):
	pred_input_counter = generated_counter[i: SEQ_LEN+i]
	pred_input_counter = np.reshape(pred_input_counter, (1, len(pred_input_counter)))
	pred_input_motifs = generated_motifs[i: SEQ_LEN+i]
	pred_input_motifs = np.reshape(pred_input_motifs, (1, len(pred_input_motifs)))
	pred_input_form = generated_form[i: SEQ_LEN+i]
	pred_input_form = np.reshape(pred_input_form, (1, len(pred_input_form)))
	...
	predicted_melody, predicted_rmid, predicted_lmid, predicted_bass = predict_notes(pred_input_melody, pred_input_rmid, pred_input_lmid,
																					pred_input_bass, pred_input_counter, pred_input_motifs, pred_input_form)
...
def add_notes_to_stream(generated_part, output_stream):
	"""Add predicted notes to a Music21 stream for a part.

	This is a reverse process of obtaining note attributes from
	midi files. Here, an additional step is done to ensure that 
	the holds/rests following note midi values are valid.

	Parameters:
	------------
	generated_part (list): list of predicted notes for a part
	output_stream: the Music21 stream object
...
output_stream.write('midi', 'model1_sample.mid')
