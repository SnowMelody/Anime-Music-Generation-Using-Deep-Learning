...
def create_model(num_melody, num_rmid, num_lmid, num_bass, seq_len, dropout):
	...
	lstm_layer = tf.keras.layers.LSTM(128, return_sequences = True)(merge_layer)
	batch_norm = tf.keras.layers.BatchNormalization()(lstm_layer)
	dropout_layer = tf.keras.layers.Dropout(dropout)(batch_norm)
	lstm_layer2 = tf.keras.layers.LSTM(128, return_sequences = False)(dropout_layer)
	batch_norm2 = tf.keras.layers.BatchNormalization()(lstm_layer2)
...
def train_model(model, training_input, training_output, test_input, test_output):
	checkpoint_path = "training_checkpoints_model3/weights-{epoch:02d}-{loss:.4f}.ckpt"
	...
	with open('model3_history', 'wb') as f:
	    pickle.dump(saveObject, f)
...
rnd_num = 123123 # choose a random starting sequence as the seed
...
output_stream.write('midi', 'model3_sample.mid')
